...
def get_stock_sector(code: str) -> str:
    ...
    if code.startswith('sh68'):
        ...
        return '创业板'
    elif code.startswith('sh6'):
        return '沪主板'
    elif code.startswith('sz0'):
        return '深主板'
    elif code.startswith('bj8'):
        return '北交所'
    return '其他'


def calc_sector_strength(sector: str, stock_change: Dict[str, float]) -> float:
    """计算板块平均涨跌幅"""
...
class SimpleScreener:
    ...
    def __init__(self, tdx_path: str):
        ...
        self.sectors = ['沪主板', '深主板', '创业板', '科创板', '北交所']
    ...
    def analyze_results(self, trades: list):
        ...
        for sector in df['sector'].unique():
            ...
            print(f"  {sector}: {len(sector_df)}笔, 胜率{len(sector_df[sector_df['profit_pct']>0])/len(sector_df)*100:.1f}%")
...
def main():
    ...
    screener = SimpleScreener(tdx_path)
    screener.load_stock_data(max_stocks=2000)

    trades = screener.backtest(min_score=3)
