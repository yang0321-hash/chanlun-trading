...
function printTradeDetail(result, buyHold) {
    ...
    if (winningTrades.length > 0 && losingTrades.length > 0) {
        const totalWin = winningTrades.reduce((s, t) => s + t.profit, 0);
        const totalLoss = Math.abs(losingTrades.reduce((s, t) => s + t.profit, 0));
...
function runBacktest() {
    console.log('='.repeat(90));
    console.log(' sz301062 回测报告');
    console.log('='.repeat(90));

    const csvFile = path.join(__dirname, 'test_output', 'sz301062.day.csv');
    ...
    // 查询股票名称
    const stockName = '301062'; // 用户可以用真实名称替换

    const result = backtestDetailed('sz301062', stockName, data, atr, macdData, fractals);
