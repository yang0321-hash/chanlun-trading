"""
ML训练数据收集 — 从TDX日线提取买点信号+5日收益

用法:
  python -m backtest.ml_collect_training              # 300样本股
  python -m backtest.ml_collect_training --all         # 全量A股
"""
import os, sys, time
sys.path.insert(0, '.')
...
def collect_signals(codes):
    """遍历所有股票，收集买点信号+5日收益"""
    all_signals = []
    daily_map = {}

    for ri, raw in enumerate(codes):
        hc = to_hs(raw)
        name = _names.get(raw, '')
        if 'ST' in name:
            continue
        try:
            df = hs.get_kline(hc, period='daily')
            if df is None or len(df) < 200:
                continue
            price = float(df['close'].iloc[-1])
            if price < 3.0 or price > 2000:
                continue
    ...
    return all_signals, daily_map


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--all', action='store_true', help='全量A股')
    ...
    if args.all:
        ...
        print(f'全量模式: {len(codes)}只')
    else:
        with open(SAMPLE) as f:
            codes = [l.strip() for l in f if l.strip()]
        print(f'样本模式: {len(codes)}只')

    signals, daily_map = collect_signals(codes)
    print(f'\n收集完成: {len(signals)}个信号, {len(daily_map)}只股票 ({time.time()-t0:.0f}s)')
    ...
    print(f'数据集: {len(dataset)}行 × {len(dataset.columns)}列')
