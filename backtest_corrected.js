...
function runAllBacktests() {
    console.log('='.repeat(90));
    console.log(' 极简优化缠论策略 - 详细回测报告 (修正版)');
    console.log('='.repeat(90));

    // 修正股票名称
    const stocks = [
        { code: 'sz002600', name: '领益智造', sector: '消费电子' },  // 修正
        { code: 'sh600497', name: '驰宏锌锗', sector: '有色金属' },  // 添加正确代码
        ...
        { code: 'sz300750', name: '宁德时代', sector: '动力电池' }
    ];
    ...
    for (const stock of stocks) {
        ...
        allResults.push({ ...result, buyHold, sector: stock.sector });
    }
    ...
    console.log('\n┌────────────┬──────────┬─────────┬─────────┬─────────┬─────────┬─────────┐');
    console.log('│ 股票       │ 板块     │ 买入持有 │ 策略    │ 超额    │ 交易数  │ 胜率    │');
    console.log('├────────────┼─────────┼─────────┼─────────┼─────────┼─────────┼─────────┤');

    for (const r of allResults) {
        const excess = r.trades.reduce((s, t) => s + t.profit, 0) - r.buyHold;
        const mark = excess > 0 ? '+' : '';
        const winRate = r.trades.length > 0 ? (r.trades.filter(t => t.profit > 0).length / r.trades.length * 100).toFixed(0) : '0';
        console.log(`│ ${r.stockCode.padEnd(10)} │ ${r.sector.padStart(7)} │ ${r.buyHold.toFixed(5).padStart(7)} │ ${r.trades.reduce((s, t) => s + t.profit, 0).toFixed(2).padStart(7)} │ ${mark}${excess.toFixed(2).padStart(6)} │ ${r.trades.length.toString().padStart(7)} │ ${winRate.padStart(6)}% │`);
    }
