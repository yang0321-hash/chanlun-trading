...
class SignalEngine:
    ...
    def __init__(self):
        ...
        self.cooldown_bars = 1              # v4基线3 → 1（最有效改进）
        self.time_stop_bars = 60            # v4基线45 → 60
        self.min_hold_before_sell = 7
        self.max_stop_pct = 0.25

        # 仓位参数（v4优化方案A）
        self.min_position = 0.10
        self.max_position = 0.30            # v4基线0.25 → 0.30
        self.base_position = 0.15           # v4基线0.12 → 0.15

        # 移动止损（v4优化方案A）
        self.trailing_start = 0.06          # v4基线0.08 → 0.06
        self.trailing_distance = 0.05       # v4基线0.04 → 0.05
    ...
    def _generate_single(self, code: str, df: pd.DataFrame) -> pd.Series:
        ...
        for i in range(120, n):
            ...
            if position > 0:
                ...
                # 30分钟确认强度因子（仓位调节，非硬过滤）
                # 双级别共振 → 仓位加成；无30分钟确认 → 仓位打折但不拒绝
                if min30_buy_ok and min30_strength > 1.0:
                    min30_factor = min(min30_strength, 1.3)  # 共振加成最多30%
                elif min30_buy_ok:
                    min30_factor = 1.0     # 30分钟确认，标准仓位
                else:
                    min30_factor = 0.85    # 30分钟不确认，仓位打85折（不拒绝入场）
    ...
    def _compute_min30_confirmation(self, code: str, daily_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        # 对每个交易日，检查该日±3天的30分钟K线是否有确认信号
        lookback_days = 3  # 搜索窗口扩大到前后3天
        for i in range(n):
            dt = daily_df.index[i]
            ...
            try:
                # 找到该日±3天范围内的30分钟K线
                start_date = (dt_ts - pd.Timedelta(days=lookback_days)).strftime('%Y-%m-%d')
                end_date = (dt_ts + pd.Timedelta(days=lookback_days)).strftime('%Y-%m-%d')
                dates = min30_df.index.strftime('%Y-%m-%d')
                ...
                for idx in day_min30.index:
                    ...
                    if min30_bi_buy.loc[idx]:
                        # 30分钟向下笔结束 = 买入确认（放宽：笔结束即确认，MACD作为强度因子）
                        buy_found = True
                        # 检查MACD状态计算确认强度
                        ...
                        if macd_idx > 0:
                            ...
                            if dif_now > dif_prev:
                                strength += 0.05  # DIF上翘
                            # 金叉特别加成
                            if (dif_now > m_dea.iloc[macd_idx] and dif_prev <= m_dea.iloc[macd_idx - 1]):
                                strength += 0.1   # DIF上穿DEA
