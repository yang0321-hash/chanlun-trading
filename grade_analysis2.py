#!/usr/bin/env python3
"""周线Grade A vs D 回测对比 - strongest信号过滤版"""
...
# 过滤到 strongest 信号 (与 v23_tp_compare 一致)
...
sig_strong = sig_df[fib.isin(['strong','strongest'])].copy()
print(f"strongest+strong 信号: {len(sig_strong)}")
...
score_dist = sig_strong['week_score'].value_counts().sort_index()
print("\nWeek score distribution (strongest信号):")
for s in sorted(score_dist.index): print(f"  score={s}: {score_dist[s]}")
A = (sig_strong['week_score']>=4).sum(); D = (sig_strong['week_score']<=1).sum(); BC = len(sig_strong)-A-D
print(f"\nA级(score≥4): {A} ({A/len(sig_strong)*100:.1f}%)")
print(f"B/C级(2~3):  {BC} ({BC/len(sig_strong)*100:.1f}%)")
print(f"D级(score≤1): {D} ({D/len(sig_strong)*100:.1f}%)")

# 简化止盈回测核心 (same as v23_tp_compare)
...
print("\n" + "="*70)
print(f"{'Group':<22} {'N':>6} {'Sharpe':>7} {'DD%':>6} {'WR%':>5} {'均盈%':>7} {'总收益%':>10}")
print("-"*75)
...
for label, mask in [
    ...
    # 极端对比
    ('A级+做多沪指',       (sig_strong['week_score']>=4)),  # same as A
    ('D级+做多沪指',       sig_strong['week_score']<=1),
]:
    ...
    if r:
        ...
        tot_str = f"{r['total_ret']:.0f}" if abs(r['total_ret']) < 1e10 else "inf"
        print(f"{label:<22} {r['n']:>6} {r['sharpe']:>7.2f} {r['max_dd']:>6.1f} {r['win_rate']:>5.0f} {r['avg_pnl']:>+7.2f} {tot_str:>10}")
