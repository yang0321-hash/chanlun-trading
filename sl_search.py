...
print(f"strongest: {len(sig_strong)}")

# 向量化一次性计算daily_state和monthly_sl（避免逐行apply）
print("计算日线状态...")
date_series = pd.to_datetime(sig_strong['date'])
daily_states = []
for d in date_series:
    daily_states.append(get_daily_state(d))
sig_strong = sig_strong.copy()
sig_strong['daily_state'] = daily_states
print("计算月度SL...")
month_series = date_series.dt.to_period('M')
monthly_sl_vals = []
for m in month_series:
    monthly_sl_vals.append(get_monthly_sl((m+1).to_timestamp() - pd.Timedelta(days=1)))
sig_strong['monthly_sl'] = monthly_sl_vals
print(f"月度SL: {len(set(monthly_sl_vals))} 个不同值")
...
print(f"有效: {len(records)} (skip {bad})")
...
# 回测函数
def bt(sl_1buy, sl_2buy, sl_3buy, bm=1.0, bem=1.0, nm=1.0):
    pnls = []
